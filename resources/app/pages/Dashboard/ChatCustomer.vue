    ...
    <div class="wrap">
        ...
        >
            <div class="trezo-card-content relative">
                ...
                >
                    <ul class="flex-col flex ltr:pl-[25px] rtl:pr-[25px]">
                        <li
                            v-for="message in messages"
                            :key="message.id"
                            :class="{
                                'relative w-full md:w-[90%] ltr:text-right ltr:mr-auto':
                                    authStore.user &&
                                    authStore.user.id === message.user_id,
                                'relative w-full md:w-[90%] ltr:text-right md:ltr:mr-auto':
                                    !authStore.user ||
                                    authStore.user.id !== message.user_id,
                            }"
                        >
        ...
        <!-- <div class="messages">
            <div v-for="m in messages" :key="m.id" class="msg">
                <b>{{ m.user.first_name }}:</b> {{ m.message }}
            </div>
        </div>
        <div class="input">
            <input
...
const listen = () => {
    ...
    channel = window.Echo.private(`room.${roomId.value}`).listen(
        ".MessageSent",
        (e) => {
...
const send = async () => {
    ...
    await axios.post("/api/messages", { message: text.value });
    messages.value.push({
        id: Date.now(),
        message: text.value,
        user: { name: "Me" },
...
onMounted(async () => {
    const room = await axios.get("/api/my-room");
