<template>
  <div
    :class="[
      'sales-navbar bg-white dark:bg-dark fixed top-0 right-0 left-0 transition-all h-auto z-[5] py-[20px] md:py-[30px] lg:py-[35px]',
      className,
      { 'is-sticky': isSticky },
    ]"
    id="navbar"
  >
    ...
    >
      ...
      >
        ...
        >
          <img
            src="@/assets/images/sales/logo.svg"
            alt="logo"
            class="inline-block dark:hidden"
          />
          <img
            src="@/assets/images/sales/white-logo.svg"
            alt="logo"
        ...
        <div class="hidden lg:flex items-center grow basis-full basis-auto">
          <ul class="flex mx-auto flex-row gap-[30px] xl:gap-[40px]">
            <li v-for="(item, index) in navItems" :key="index">
              <template v-if="item.isExternal">
                <a
                  :href="item.path"
                  class="font-medium transition-all text-[#7a857d] hover:text-[#06201b] hover:dark:text-white cursor-pointer"
                  target="_blank"
              ...
              <template v-else>
                <RouterLink
                  ...
                  class="font-medium transition-all text-[#7a857d] hover:text-[#06201b] hover:dark:text-white cursor-pointer"
                  :class="{
                    'text-[#06201b] dark:text-white': isActive(item.path),
                    '': !isActive(item.path),
                  }"
                >
                  {{ item.name }}
          ...
          <a
            ...
            class="inline-block font-medium md:text-base rounded-[7px] bg-lime-500 text-black py-[10.5px] md:py-[11.5px] px-[22px] md:px-[25px] transition-all hover:bg-lime-600"
            target="_blank"
          >
            Get Started
          </a>
        ...
        >
          ...
          <a
            href="https://trezo-twcss.envytheme.com/"
            class="inline-block font-medium md:text-base rounded-[7px] bg-lime-500 text-black py-[10.5px] md:py-[11.5px] px-[22px] md:px-[25px] transition-all hover:bg-lime-600 mt-[15px]"
            target="_blank"
          >
            Get Started
          </a>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const navItems = ref<NavItem[]>([
      ...
      { name: "Use Cases", path: "/use-cases", isExternal: false },
